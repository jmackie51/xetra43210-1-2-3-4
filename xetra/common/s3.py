...
class S3BucketConnector():
    ...
    def read_csv_to_df(self, key, decoding = 'utf-8', delimeter = ','):
        """
        listing all files with a prefix on the S3 bucket
        ...
        data_frame = pd.read_csv(data, delimiter=delimeter)
        return data_frame

    def write_df_to_s3(self, key: str, data_frame: pd.DataFrame, file_format: str):
        """
        ...
        if file_format == S3FileTypes.PARQUET.value:
            ...
            return self.__put_object(out_buffer, key)
        self._logger.info('The file format %s in not supported to be written to s3!', file_format)
